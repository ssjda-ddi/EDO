...
namespace EDO.Core.View
{
    public class DataGridRollbackOnUnfocusedBehaviour
    {
         ...
         DependencyObject depObj, DependencyPropertyChangedEventArgs e)
        {
            ...
            if ((bool)e.NewValue)
            {
                //**Confirm if this event handler is needed. If stays enabled the contents disapper when DataGrid exists on tab control
                //It's seems to be no problem even if I move the key focus to external without it.
                datagrid.LostKeyboardFocus += RollbackDataGridOnLostFocus; //Bad after all, it gets abend in Question Scheme window without it
